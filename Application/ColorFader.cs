namespace SpeakerTimer.Application
{
    ...
    public static class ColorFader
    {
        ...
        public static Color BoundedExponential(FadeItem init, FadeItem dest, FadeItem current)
        {
            ...
            if (fadeIndex < steps)
            {
                var scale = init.Time + steps;
                var func = 1 - Math.Exp(fadeIndex - steps);
                ...
                var r = (int)(((initR - destR) * func) + destR);
                var g = (int)(((initG - destG) * func) + destG);
                var b = (int)(((initB - destB) * func) + destB);
